...
function classCalculator(answer) {
    ...
    } else if (!answer.isToggled && answer.gameFinished && props.text) {
    ...
    return (
        ...
        {/*The function passed down as handleClick is fed the id and index back which enables the correct rendering of the answer buttons */}
            <button className={classCalculator(props)} onClick={() => props.handleClick(props.id, props.index, props.text)}>{props.text}</button>
        </>
    )
}